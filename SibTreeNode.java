...
class SibTreeNode extends TreeNode {
	  ...
	  if (isValidNode()){
		  ...
		  if (size == 0 ){
			  ...
			  if (c <= 1){
				  ...
				  ((SibTreeNode)this.child(size)).nextSibling = node;
			  } else {
				  node.nextSibling = ((SibTreeNode)this.child(c));
				  ((SibTreeNode)this.child(c-1)).nextSibling = node;
			  }
		  }
		  myTree.size ++;
	  }else {
  ...
  public void removeLeaf() throws InvalidNodeException {
    // FILL IN YOUR SOLUTION TO PART III HERE.
	  if (isValidNode()){
		  if ( this.firstChild != null && this.firstChild.isValidNode()){
			  return;
		  } else {
			  if ( this == myTree.root){
				  this.valid =false;
			  ...
			  else if(this == this.parent.firstChild){
				  this.parent.firstChild = this.nextSibling;
				  this.valid =false;
				  myTree.size --;
			  }
			  else if(this.nextSibling == null || !this.nextSibling.isValidNode()){
				  this.valid =false;
				  myTree.size --;
			  }
			  else {
				  SibTreeNode node = this.parent.firstChild;
